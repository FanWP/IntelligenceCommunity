...
//    @property(nonatomic,strong) UILabel *isSupportBarterLabel;
    ...
    _isSupportBarterLabel.textColor = [UIColor grayColor];
    _isSupportBarterLabel.textAlignment = NSTextAlignmentLeft;
    _isSupportBarterLabel.font = UIFontNormal;
    ...
    _isSupportBarterImageView = [[UIImageView alloc] init];
    _isSupportBarterImageView.contentMode = UIViewContentModeCenter;
    _isSupportBarterImageView.image = [UIImage imageNamed:@"compose_emoticonbutton_background_highlighted"];
    [self addSubview:_isSupportBarterImageView];
    [_isSupportBarterImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(_userImageView.mas_centerY);
        make.right.equalTo(_isSupportBarterLabel.mas_left).offset(-5);
...
//已售
    ...
    img.mj_y = _communityImageView.mj_y + 65;
    self.alreadySell = img;
    [self addSubview:img];
    _areadlySaleView = [[MaterialsAreadlySaleView alloc] init];
    _areadlySaleView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    _areadlySaleView.alpha = 0.7f;
    _areadlySaleView.hidden = YES;
    [_communityImageView addSubview:_areadlySaleView];
    [_areadlySaleView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(_communityImageView.mas_centerY);
        make.left.equalTo(_communityImageView.mas_left).offset(0);
        make.width.mas_equalTo(_communityImageView);
...
//    @property(nonatomic,strong) UILabel *oldPriceLabel;
    ...
    _oldPriceLabel.font = UIFontSmall;
...
#warning 评论按钮、数量、点赞按钮的大小以点赞数量控件为基准动态改变
    ...
    [_commentsButton setImage:[UIImage imageNamed:@"compose_emoticonbutton_background_highlighted"] forState:UIControlStateNormal];
...
//    @property(nonatomic,strong) UILabel *thumbUpCountLabel;
    ...
    [_thumbUpButton setImage:[UIImage imageNamed:@"compose_emoticonbutton_background_highlighted"] forState:UIControlStateNormal];
...
//    @property(nonatomic,strong) UILabel *communityContentLabel;
    ...
    _communityContentLabel.font = UIFontNormal;
    [self addSubview:_communityContentLabel];
    [_communityContentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
...
-(void)layoutSubviews{
    ...
    _lineLayer.frame = CGRectMake(0, self.bounds.size.height-5, self.width, 5);
}
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
...
//    [self.communityImageView load:freeArticleModel.images placeholderImage:[UIImage imageNamed:@"compose_emoticonbutton_background_highlighted"]];
    ...
    self.priceLabel.text = freeArticleModel.price;
    self.oldPriceLabel.text = freeArticleModel.srcPrice;
    
    
    self.thumbUpCountLabel.text = @"iw35tip3qw";
    self.thumbUpCountLabel.text = @"8640326";
    
    self.commentCountLabel.text = @"754923";
